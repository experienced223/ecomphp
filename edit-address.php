...
if(isset($_POST) & !empty($_POST)){
		...
		$zip = filter_var($_POST['zipcode'], FILTER_SANITIZE_NUMBER_INT);
            ...
            $check_usermeta_exist = "
            SELECT id FROM usersmeta
            WHERE uid='$uid'
            ";
            $usermeta = mysqli_query($connection, $check_usermeta_exist) or die(mysqli_error($connection));
            if(mysqli_num_rows($usermeta)<=0){

                $insert_into_usersmeta_table =
                    "
                    INSERT INTO usersmeta (uid, country, firstname, lastname,address1,address2,city,state,zip,company,mobile)
                    VALUES ('$uid', '$country', '$fname', '$lname','$address1','$address2','$city','$state','$zip','$company','$phone') 
                    ";

                $insert_in_usermeta = mysqli_query($connection, $insert_into_usersmeta_table) or die(mysqli_error($connection));
                if($insert_in_usermeta){
                    //echo "inserted";
                }
            }

			$usql = "UPDATE usersmeta SET 
                        country='$country', firstname='$fname', lastname='$lname', 
                        address1='$address1', address2='$address2', city='$city', 
                        state='$state',  zip='$zip', company='$company', mobile='$phone' 
                        WHERE uid=$uid";
	...
	<section id="content">
		<div class="content-blog">
					<div class="page_header text-center">
						<h2>Update Profile Info, Address</h2>
					</div>
<form method="post">
<div class="container">
			<div class="row">
